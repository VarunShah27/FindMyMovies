...
const popularMovies = [
  ...
  "Pulp Fiction", "Carry on Jatta", "Forrest Gump", "The Matrix", "Ardaas",
  "Goodfellas", "Interstellar", "Dhamaal", "PK", "Goreyan nu Daffa Karo",
  "The Green Mile", "Manje Bistre", "Housefull 2", "Sikander 2", "Yaar Anmulle",
  "F1", "Gladiator", "Joker", "Whiplash", "The Prestige", "Taarzan: The Wonder Car",
...
function App() {
  ...
  useEffect(() => {
    ...
    const promises = selectedMovies.map((title) =>
      axios.get(`${BASE_URL}/search/movie`, { params: { api_key: API_KEY, query: title, language: "en-US" } })
        .then(res => {
          const movie = res.data.results[0]; // fallback to first result
  ...
  const search = (term) => {
    if (!term) return;
    axios.get(`${BASE_URL}/search/movie`, { params: { api_key: API_KEY, language: "en-US", query: term } })
      .then(({ data }) => {
  ...
  const openPopup = async (id) => {
    try {
      ...
      setState(prev => ({
        ...prev,
        selected: { ...details, director, cast }
      }));
