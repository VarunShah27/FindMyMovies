...
function App() {
  ...
  // Fetch curated + trending home page movies
  useEffect(() => {
    // Fetch curated movies
    const curatedPromises = popularMovies.slice(0, 8).map(title =>
      axios.get(`${BASE_URL}/search/movie`, { params: { api_key: API_KEY, query: title, language: "en-US" } })
        .then(res => {
          const movie =
          ...
          return {
            ...movie,
            poster_path: movie.poster_path ? IMAGE_BASE + movie.poster_path : "https://via.placeholder.com/342x513?text=No+Image"
          };
        })
    );

    // Fetch trending TMDb movies
    const trendingPromise = axios.get(`${BASE_URL}/trending/movie/week`, { params: { api_key: API_KEY } })
      .then(res => res.data.results.slice(0, HOME_LIMIT - 8).map(movie => ({
        ...movie,
        poster_path: movie.poster_path ? IMAGE_BASE + movie.poster_path : "https://via.placeholder.com/342x513?text=No+Image"
      })));

    Promise.all([...curatedPromises, trendingPromise])
      .then(results => {
        const curated = results.slice(0, 8).filter(Boolean);
        const trending = results[results.length - 1];
        setState(prev => ({ ...prev, results: [...curated, ...trending] }));
      })
  ...
  const search = (term) => {
    if (!term) return;
    axios.get(`${BASE_URL}/search/movie`, { params: { api_key: API_KEY, query: term, language: "en-US" } })
      .then(({ data }) => {
        if (!data.results || data.results.length === 0) {
          ...
          popularMovies.forEach(movie => {
            ...
            if (distance < minDistance && distance <= 3) { minDistance = distance; bestMatch = movie; }
          });
  ...
  // Popup with director + cast
  const openPopup = async (id) => {
    try {
      const { data: details } = await axios.get(`${BASE_URL}/movie/${id}`, { params: { api_key: API_KEY, language: "en-US" } });
      const { data: credits } = await axios.get(`${BASE_URL}/movie/${id}/credits`, { params: { api_key: API_KEY } });
      ...
      setState(prev => ({
        ...
        selected: {
          ...
          director,
          cast,
          poster_path: details.poster_path ? IMAGE_BASE + details.poster_path : "https://via.placeholder.com/342x513?text=No+Image"
        }
      }));
